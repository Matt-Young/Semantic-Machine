...
int DELETE_TABLE(TABLE *table) {
  ...
  sprintf_s(buff,200,Sql_delete,tables->name);
  return(triple(t,0));
}
...
TRIPLE install(int index,int opid,int format) {
  ...
  sprintf_s(buff,200,"%d",opid);
  status = triple(t,0);
  sprintf_s(buff,200,pre_installed[format].sql, tables[index].name);
  t.pointer++; 
  status = triple(t,0);
  for(i=0;i < 4;i++) {
    sprintf_s(buff,200,"%d", pre_installed[format].parm[i]);
...
int init_gfun() {
  int status;
     status = sqlite3_create_function_v2(m.db,GFUN,2,SQLITE_UTF8 ,0,gfunction,0,0,NULL);
...
int bind_default(sqlite3_stmt *stmt,TRIPLE top,int j,int i) {
  ...
  if(t ==  SQLITE_INTEGER)
      ...
      status = sqlite3_bind_text(stmt,i+1,(char *) p,strlen((char *) p),0);
  else
...
int local_handler(TRIPLE t) {
  sprintf_s(buffer + strlen(buffer),200," %s ,",t.key);
  return SQLITE_OK;
}
...
int bind_schema(sqlite3_stmt *stmt,TRIPLE top) {
  ...
  g->row = g->start;
  index = 0;
  sprintf_s(buffer,200,"select ");
  triple(table->pop_triple,local_handler);
  sprintf_s(buffer,200,"from %s where (gfun(0,rowid) == rowid);",
    NAME(g->table));
